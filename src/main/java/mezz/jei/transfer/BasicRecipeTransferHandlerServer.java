...
public final class BasicRecipeTransferHandlerServer {
	...
	public static void setItems(EntityPlayer player, Map<Integer, Integer> slotIdMap, List<Integer> craftingSlots, List<Integer> inventorySlots, int maxTransfer,
								boolean requireCompleteSets, @Nullable Map<Integer, Integer> itemCounts) {
		...
		for (Map.Entry<Integer, Integer> entry : slotIdMap.entrySet()) {
			...
			stack.setCount(itemCounts == null ? 1 : itemCounts.get(entry.getKey()));
		...
		for (int craftingSlotNumberIndex = 0; craftingSlotNumberIndex < craftingSlots.size(); craftingSlotNumberIndex++) {
			...
			ItemStack item = craftingSlot.getStack();
			if (!item.isEmpty() && craftingSlot.isItemValid(item)) {
	...
	public static void setItems(EntityPlayer player, Map<Integer, Integer> slotIdMap, List<Integer> craftingSlots, List<Integer> inventorySlots, boolean maxTransfer,
		...
		setItems(player, slotIdMap, craftingSlots, inventorySlots, maxTransfer ? Integer.MAX_VALUE : 1, requireCompleteSets, null);
	}

	public static void setItems(EntityPlayer player, Map<Integer, Integer> slotIdMap, List<Integer> craftingSlots, List<Integer> inventorySlots, int maxTransfer,
								boolean requireCompleteSets) {
		setItems(player, slotIdMap, craftingSlots, inventorySlots, maxTransfer, requireCompleteSets, null);
	}
		...
		while (true) { // for each set
			...
			for (Map.Entry<Integer, ItemStack> entry : required.entrySet()) { // for each item in set
				...
				if (!itemFound || resultItemStackLimitReached) {
					...
					ItemStack removedItemStack = slot.decrStackSize(entry.getValue().getCount());
	...
	@Nullable
	private static Slot getSlotWithStack(EntityPlayer player, Container container, ItemStack stack, List<Integer> craftingSlots, List<Integer> inventorySlots) {
		Slot slot = getSlotWithStack(player, container, craftingSlots, stack);
		if (slot == null) {
			slot = getSlotWithStack(player, container, inventorySlots, stack);
		}

		return slot;
	}

	private static int addStack(EntityPlayer player, Container container, Collection<Integer> slotIndexes, ItemStack stack) {
		int added = 0;
		...
		for (final Integer slotIndex : slotIndexes) {
			if (slotIndex >= 0 && slotIndex < container.inventorySlots.size()) {
				...
				final ItemStack inventoryStack = slot.getStack();
				if (inventoryStack.isEmpty() && slot.isItemValid(stack)) {
	 ...
	 */
	@Nullable
	private static Slot getSlotWithStack(EntityPlayer player, Container container, Iterable<Integer> slotNumbers, ItemStack itemStack) {
